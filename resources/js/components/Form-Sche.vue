<template>
  <form @submit.prevent="postScheduleForm()">
    ...
    <div class="progress-bar">
      <div
        class="progress-fill"
        :class="['w-' + progressfill]"
      >{{ progressbar }} {{ $t("schedule-form-completed") }}</div>
    </div>
    <div class="form-tab tab-1" v-if="step == 1">
      <fieldset>
        <legend class="g-text">{{ $t("schedule-form-personal") }}</legend>
        <label for="name">{{ $t("schedule-form-name-surname") }}</label>
        <input v-model.trim="$v.formFullName.$model" type="text" placeholder="Name and Surname" />
        ...
        <input type="text" placeholder="Example:Service/Food" v-model.trim="$v.formIndustry.$model" />
    ...
    <div class="form-tab tab-2" v-if="step == 2">
      <fieldset>
        <legend class="g-text">{{ $t("schedule-form-account") }}</legend>
        <label for="project-title">
          {{
          $t("schedule-form-project-title")
          }}
        </label>
        <input type="text" placeholder="Project Title" v-model.trim="$v.formProjectTitle.$model" />
        ...
        ></textarea>
        <div class="form-error" v-if="formError">There is an error looking in your form fields.</div>
      </fieldset>
    </div>
    <div class="form-tab tab-3" v-if="step == 3">
      <fieldset>
        <legend class="g-text">{{ $t("schedule-form-picture") }}</legend>
        <h4 class="g-text">
        ...
        <ul>
          ...
          <li class="preview-item" v-for="file in files" :key="file.id">
            <img class="preview-img" :src="file.blob" width="150" height="100" />
            <span>Filename:</span>
    ...
    <div class="steps-form">
      <div class="step-form-item step-item-1" :class="{ 'step-active': stepActive1 }"></div>
      <div class="step-form-item step-item-2" :class="{ 'step-active': stepActive2 }"></div>
      <div class="step-form-item step-item-3" :class="{ 'step-active': stepActive3 }"></div>
      <div class="step-form-item step-item-4" :class="{ 'step-active': stepActive4 }"></div>
    </div>
    <div v-if="step == 1 || step == 2 || step == 3 || step == 4" class="button-set">
      <a
        @click="prev"
    ...
    <button type="submit" v-if="step == 4" class="btn-lg btn-primary pointer">Submit</button>
...
export default {
  ...
  methods: {
    ...
    postScheduleForm() {
      if (this.step == 4 && this.$v.formScheduleDate.$invalid) {
        this.formError = true;
      } else {
    ...
    next() {
      ...
      if (this.step < 4) {
        ...
        if (this.step == 1 && this.$v.step1GroupValidate.$invalid) {
          this.formError = true;
        } else if (this.step == 2 && this.$v.step2GroupValidate.$invalid) {
          this.formError = true;
        } else {
          this.formError = false;
          this.step++;
