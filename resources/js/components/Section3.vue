  ...
  <section class="page-section section-3 snap-item">
    ...
    <div class="recent-news">
      <div v-for="recentblog in recentBlogs" class="news-item">
        <img src="http://placehold.it/690x527" loading="lazy" alt />
        ...
        <div class="news-info">
          ...
          <span>{{ $moment(recentblog.created_at).format("l") }}</span>
          <i class="fa fa-user g-text" aria-hidden="true"></i>
          <span>{{recentblog.author}}</span>
          <i class="fa fa-tag g-text" aria-hidden="true"></i>
          <span>{{categories[recentblog.category_id - 1].name }}</span>
        </div>
        <router-link
          :to="{ name: 'BlogDetail', params: { blogId: recentblog.id }}"
